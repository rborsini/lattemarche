...
export default class EditazionePrelievoModal extends Vue {
  ...
  @Watch('prelievoLatte')
  onPrelievoLatteChanged(){

    var p = this.prelievoLatte;
    if(p.Lat && p.Lng) {
      this.showMap = true;
      var center = new Position(p.Lat, p.Lng);            
      var marker = new Marker(p.Lat, p.Lng, p.IdAllevamento + " - " + p.DataPrelievoStr);
