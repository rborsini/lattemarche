<template>
  <div>
    ...
    <notification-dialog
      ...
      v-on:ok="window.location = '/TipiLatte'"
    ></notification-dialog>
...
export default class TipiLatteIndexPage extends Vue {
  ...
  public mounted() {
    ...
    this.tipiLatteService.index().then(response => {
  ...
  public onDataLoaded() {
    $(".edit").click(event => {
      ...
      this.tipiLatteService.details(rowId).then(response => {
  ...
  // rimozione tipoLatte
  public onRemove() {
    this.tipiLatteService.delete(this.idTipoLatte).then(response => {
