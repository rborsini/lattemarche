<template>
	<div class="container-fluid p-0">
		...
		<notification-dialog ref="removedDialog" :title="'Conferma rimozione'" :message="'Utente rimosso correttamente'" v-on:ok="window.location = '/Utenti'"></notification-dialog>

		<!-- Pannello modale conferma eliminazione -->
		<confirm-dialog ref="confirmDeleteDialog" :title="'Conferma eliminazione'" :message="'Sei sicuro di voler rimuovere l\'utente selezionato?'" v-on:confirmed="onRemove()"></confirm-dialog>
		...
		<div class="jumbotron">
			<div class="row pt-1">
				...
				<div class="col-3">
					<select2 id="cy-profilo" class="form-control" :placeholder="'-'" :options="profilo.Items" :value-field="'Value'" :text-field="'Text'" :value.sync="parameters.IdProfilo" v-on:value-changed="onProfiloValueChanged" />
				</div>
			</div>
		</div>

		<!-- Tabella -->
		<data-table ref="table" class="pt-4" :options="tableOptions" v-on:data-loaded="onDataLoaded">
			<!-- Toolbox -->
...
export default class UtentiIndexPage extends Vue {
	...
	private initTable(): void {
		...
		options.ajax = {
			url: "/api/utenti/search?" + this.parameters.toUrlQueryString(),
			type: "POST",
		};
